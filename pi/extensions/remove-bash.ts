...
export default function (pi: ExtensionAPI) {
    ...
    // - when navigating the session tree (branch switching)
    pi.on("session_start", async (_event, _ctx) => applyToolFilter());
    pi.on("session_tree", async (_event, _ctx) => applyToolFilter());
}
