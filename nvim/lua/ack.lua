...
local function check_dispatch()
    if vim.g.ack_use_dispatch == nil then vim.g.ack_use_dispatch = 0 end
...
local function init(cmd)
    ...
    if vim.g.ack_use_dispatch == 1 and using_loclist then
        warn(
            'Dispatch does not support location lists! Proceeding with quickfix...')
        using_loclist = false
    end
end

-- Are we finding matching files, not lines? (the -g option -- :AckFile)
local function is_searching_filepaths() return searching_filepaths end

-- Were we invoked with a :LAck command?
local function is_using_loclist() return using_loclist end
...
local function highlight(args)
    if vim.g.ackhighlight == 0 then return end

    -- Extract search pattern from args
    local pattern = vim.fn.matchstr(args,
                                    "\\v(-)\\@<!(\\<)\\@<=\\w+|['\"]\\zs.{-}\\ze['\"]")
...
local function apply_mappings()
    if not is_using_list_mappings() or vim.bo.filetype ~= 'qf' then return end
    ...
    if vim.g.ack_autoclose == 1 then
        ...
        for key, mapping in pairs(vim.g.ack_mappings) do
            vim.keymap.set('n', key, mapping .. closemap,
                           {buffer = true, silent = true})
        end
        vim.keymap.set('n', '<CR>', '<CR>' .. closemap,
                       {buffer = true, silent = true})
    else
        for key, mapping in pairs(vim.g.ack_mappings) do
    ...
    if vim.g.ackpreview then
        vim.keymap.set('n', 'j', 'j<CR><C-W><C-P>',
                       {buffer = true, silent = true})
        vim.keymap.set('n', 'k', 'k<CR><C-W><C-P>',
                       {buffer = true, silent = true})
...
local function search_with_dispatch(grepprg, grepargs, grepformat)
    ...
    if is_searching_filepaths() then final_grepprg = grepprg .. ' -g' end
    ...
    for _, bufnr in ipairs(unique_files) do
        ...
        if name ~= '' then
            table.insert(filenames,
                         vim.fn.shellescape(vim.fn.fnamemodify(name, ':p')))
        end
    end
...
function M.ShowResults()
    local handler = is_using_loclist() and vim.g.ack_lhandler or
                        vim.g.ack_qhandler
    vim.cmd(handler)
    apply_mappings()
    vim.cmd('redraw!')
...
function M.setup()
    ...
    if vim.g.ack_mappings then
        vim.g.ack_mappings = vim.tbl_extend("force", s_ack_mappings,
                                            vim.g.ack_mappings)
    else
        vim.g.ack_mappings = s_ack_mappings
    end

    if not vim.g.ack_qhandler then vim.g.ack_qhandler = "botright copen" end

    if not vim.g.ack_lhandler then vim.g.ack_lhandler = "botright lopen" end

    if vim.g.ackhighlight == nil then vim.g.ackhighlight = 0 end

    if vim.g.ack_autoclose == nil then vim.g.ack_autoclose = 0 end

    if vim.g.ack_autofold_results == nil then vim.g.ack_autofold_results = 0 end
