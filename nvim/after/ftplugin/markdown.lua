...
local function select_inside_fence()
    ...
    if prev == 0 then
        error("Couldn't find beginning of code fence")
    end
    -- Count how many backticks we have
    -- We can probably combine search and matchbufline, but whatever...
    local match = vim.fn.matchbufline(vim.api.nvim_get_current_buf(), "^\\(`\\{3,}\\)[^`]*$", prev, prev, {submatches = true})
    if #match == 0 then
        error("no matches")
    end
    if #match > 1 then
    ...
    if next == 0 then
        error(string.format("Couldn't find end of code fence for %d backticks", #backticks))
    end
